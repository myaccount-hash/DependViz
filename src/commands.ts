 ...
 * コマンドを登録する
 */
export function registerCommands(_context: vscode.ExtensionContext, providers: Providers): vscode.Disposable[] {
    const { settingsProvider, filterProvider, graphViewProvider, callStackProvider, analyzerManager } = providers;
    const configManager = ConfigurationManager.getInstance();
    const getAnalyzerName = (): string => analyzerManager.getActiveAnalyzerName();
    const getAnalyzerId = (): string => analyzerManager.getActiveAnalyzerId();

    const createSliceCommand = (direction: 'forward' | 'backward') => async (): Promise<void> => {
    ...
    const commands: vscode.Disposable[] = [
        ...
        vscode.commands.registerCommand('forceGraphViewer.toggleCheckbox', async (key: string) => {
            const controls = configManager.loadControls({ ignoreCache: true });
            await configManager.updateControl(key, !controls[key]);
        }),
        vscode.commands.registerCommand('forceGraphViewer.toggleCallStackEntry', async (sessionId: string) => {
            if (!sessionId) return;
            const controls = configManager.loadControls({ ignoreCache: true });
        ...
        vscode.commands.registerCommand('forceGraphViewer.selectAnalyzer', async (analyzerId: string) => {
            if (typeof analyzerId === 'string' && analyzerId.length > 0) {
                await configManager.updateControl('analyzerId', analyzerId);
            }
        }),
        vscode.commands.registerCommand('forceGraphViewer.showSliderInput', async (key: string, min: number, max: number, _step: number, currentValue: number) => {
        ...
        vscode.commands.registerCommand('forceGraphViewer.analyzeProject', async (): Promise<void> => {
        ...
        vscode.commands.registerCommand('forceGraphViewer.analyzeCurrentFile', async (): Promise<void> => {
            ...
            try {
                ...
                vscode.window.showErrorMessage(`解析失敗: ${(e as Error).message}`);
            }
        }),
        vscode.commands.registerCommand('forceGraphViewer.updateCallStack', async () => {
            try {
                ...
                vscode.window.showErrorMessage(`取得失敗: ${(e as Error).message}`);
            }
        }),
        vscode.commands.registerCommand('forceGraphViewer.removeCallStackEntry', async (item: vscode.TreeItem & { id?: string }) => {
            if (!item?.id) {
                return;
            }
            try {
                ...
                vscode.window.showErrorMessage(`削除失敗: ${(e as Error).message}`);
            }
        }),
