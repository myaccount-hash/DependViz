...
export interface HighlightedPath {
  nodes: Set<string>;
  pathLinks: Array<{ source: string; target: string }>;
}
...
// Rule types
export type NodeRule = (node: GraphNode, ctx: { state: GraphViewModel; _getTypeColor: (category: string, type?: string) => string | null }) => Partial<NodeVisualProps> | null;
export type LinkRule = (link: GraphLink, ctx: { state: GraphViewModel; _getTypeColor: (category: string, type?: string) => string | null }) => Partial<LinkVisualProps> | null;

// Slice computation result
