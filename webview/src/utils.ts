...
export function applyFilter(nodes: GraphNode[], links: GraphLink[], state: GraphViewModel): GraphData {
  const controls = state.controls;
  const typeFilters = controls.typeFilters || {};

  const isTypeVisible = (category: 'node' | 'edge', type?: string): boolean => {
...
export function computeSlice(
  ...
  const sliceLinks = new Set<GraphLink>();
  const getId = (v: string | GraphNode): string => (typeof v === 'object' ? v.id : v);

  const visit = (nodeId: string, depth: number, direction: 'forward' | 'backward'): void => {
    if (depth <= 0) return;
    links.forEach(link => {
...
function matchesSearchQuery(node: GraphNode, query: string): boolean {
  ...
  return Boolean((node.name && node.name.toLowerCase().includes(q)) ||
         (node.id && node.id.toLowerCase().includes(q)));
}

function evaluateFieldQuery(node: GraphNode, query: string): boolean {
