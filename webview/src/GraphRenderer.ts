...
abstract class GraphRenderer {
  ...
  constructor(state: GraphViewModel) {
    this.state = state;
    this.is3D = state.controls.is3DMode ?? false;
    ...
    this.nodeRules = [
      ...
      (node, ctx) => ctx.state.controls.nodeSizeByLoc && node.linesOfCode && node.linesOfCode > 0 ? {
  ...
  _getTypeColor(category: string, type?: string): string | null {
    if (!type) return null;
    const cat = category as 'node' | 'edge';
    const map = this.state.controls.typeColors?.[cat];
    if (!map) return null;
  ...
  getNodeVisualProps(node: GraphNode): NodeVisualProps {
    ...
    const props = this._applyRules(node, this.nodeRules, {
      ...
      size: this.state.controls.nodeSize
    } as NodeVisualProps);
  ...
  getLinkVisualProps(link: GraphLink): LinkVisualProps {
    ...
    const props = this._applyRules(link, this.linkRules, {
      ...
      width: this.state.controls.linkWidth
    } as LinkVisualProps);
  ...
  protected _applyColors(
    getNodeProps: (node: GraphNode) => NodeVisualProps | null,
    getLinkProps: (link: GraphLink) => LinkVisualProps | null
  ): void {
    const COLORS = this.state.controls.COLORS || {};
    const graph = this.state.graph as any;

    graph
      .nodeColor((node: GraphNode) => {
        const props = getNodeProps(node);
        const color = props?.color || COLORS.NODE_DEFAULT || '#93c5fd';
        return applyOpacityToColor(color, props?.opacity);
      })
      .linkColor((link: GraphLink) => {
        const props = getLinkProps(link);
        const color = props?.color || COLORS.EDGE_DEFAULT || '#4b5563';
        return applyOpacityToColor(color, props?.opacity);
      })
  ...
  updateGraph(options: GraphUpdateOptions = {}): void {
    ...
    const graph = this.state.graph as any;
    graph.backgroundColor(this.state.getBackgroundColor());

    const nodes = this.state.data.nodes || [];
    const links = this.state.data.links || [];
    const getNodeProps = (node: GraphNode) => this.getNodeVisualProps(node);
    const getLinkProps = (link: GraphLink) => this.getLinkVisualProps(link);
    ...
    if (reheatSimulation && graph.d3ReheatSimulation) {
      setTimeout(() => graph.d3ReheatSimulation(), 100);
    }
